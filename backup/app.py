    ...
    for idx, filename in enumerate(os.listdir(image_folder_path)):
        if filename.lower().endswith((".jpeg", ".png", ".jpg", ".webp")):
            ...
            try:
                ...
                labels.append(idx)
                known_face_names.append(filename)  # Full filename with extension
...
def recognize():
    ...
    try:
        ...
        if matched_name:
            # Full filename directly use karo (extension included)
            matched_image_name = matched_name  # Already has extension e.g. "test image.jpeg"

            result = f"Match mila! {match_pct:.1f}% confidence"
            # DB match ke liye extension hata do
            matched_name_no_ext = os.path.splitext(matched_name)[0]
            additional_info = fetch_additional_info(matched_name_no_ext)
            if os.path.exists(image_path):
                os.remove(image_path)
            return render_template('persondetail.html',
                                   result=result,
                                   matched_image=quote(matched_image_name),
